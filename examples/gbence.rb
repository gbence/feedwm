module FeedWM
  class << self
    ...
    def upgrade
      now_mtime = File.mtime(__FILE__)
      @last_mtime ||= now_mtime
      return true if @last_mtime == now_mtime
      eval(File.read(__FILE__), TOPLEVEL_BINDING)
      @last_mtime = now_mtime
    end
...
#  loop do
#    begin
#      if (_ = File.mtime(__FILE__)) != last_mtime
#        eval(File.open(__FILE__).read)
#        last_mtime = _
#      end
#      x_root.name = generate()
#    rescue
#      x_root.name = $!.inspect
#      eval %(
#        def generate
